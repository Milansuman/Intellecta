...
export async function updateEventHandler(formData: FormData){
    await updateEvent(formData.get("id") as string, formData.get("name") as string, formData.get("type") as string, formData.get("datetime") as string, formData.get("college") as string);
    revalidatePath("/events")
    redirect("/events")
}
